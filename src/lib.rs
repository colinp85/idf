...
impl Idf {
    ...
    /// Ordered iterator that will iterate the Idf in the order it was populated
    ///
    ...
    /// 
    /// for (key, value) in idf.into_ordered_iterator() {
    ...
    /// Clears the Idf object, removing all key-value pairs and resetting internal state.
    ///
    /// This function clears the underlying hashmap, resets the index (`m_idx`) to 0,
    /// and clears the ordered map (`m_ordered`). After calling this function, the Idf
    ...
    /// Returns the number of key-value pairs in the Idf object.
    ///
    /// This function returns the count of key-value pairs stored in the Idf object.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Updates the current Idf object with the values from another Idf object.
    ///
    /// This function iterates through the key-value pairs of the `other` Idf object
    /// and inserts or updates the corresponding entries in the current Idf object.
    /// Existing values are replaced with the new values, and new key-value pairs
    /// are added to the current object.
    ///
    /// # Arguments
    ///
    /// * `other` - Another Idf object to update from.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// ido1.update(&ido2);
    /// ```
    pub fn update(&mut self, other: &Idf) {
        for (key, value) in &other.m_items {
            self.m_items.insert(*key, value.clone());
        }
    }

    /// Checks if the `Idf` object contains the specified key.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns `true` if the `Idf` object contains the specified key, `false` otherwise.
    ///
    /// # Example
    ///
    /// ```
    ...
    ///
    /// ```
    /// use idf::{Idf, IdoKeyT, IdoItem};
    ///
    /// let mut idf = Idf::new();
    /// let item = IdoItem::new();
    ///
    /// idf.set_item(&42, item);
    /// ```
    ...
    /// use idf::{Idf};
    /// let mut idf = Idf::new();
    /// idf.set_string(&1, "John Doe".to_string());
    /// ```
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(string_value) = idf.get_string(&42) {
    ...
    /// if let Some(value) = idf.get_i64(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_i32(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ///     println!("Value not found or not an integer.");
    /// }
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_i16(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ///     println!("Value not found or not an integer.");
    /// }
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_i8(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ///     println!("Value not found or not an integer.");
    /// }
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_u64(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ///     println!("Value not found or not an integer.");
    /// }
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_u32(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ///     println!("Value not found or not an integer.");
    /// }
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_u16(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ///     println!("Value not found or not an integer.");
    /// }
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_u8(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ///     println!("Value not found or not an integer.");
    /// }
    ...
    /// use idf::{Idf};
    /// let idf = Idf::new();
    /// if let Some(value) = idf.get_f64(&42) {
    ///     println!("Value: {}", value);
    /// } else {
    ...
    /// # Arguments
    ///
    /// * `key` - A reference to the key of the array in the Idf object.
    ...
    /// Appends an Idf object to an array within the Idf object.
    ///
    ...
    /// The provided Idf object is then appended to the array.
    ///
    /// # Arguments
    ///
    /// * `key` - A reference to the key identifying the array within the Idf object.
    /// * `data` - The Idf object to append to the array.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// idf.append_array(&11, data);
    /// ```
    pub fn append_array(&mut self, key: &IdoKeyT, data: Idf)
    {
        if let Some(value) = self.m_items.get(key) {
    ...
    /// Deletes an item from the Idf object based on the given key.
    ///
    /// If an item with the specified key exists in the Idf object, it will be removed.
    ...
    /// Converts the Idf object to a string representation.
    ///
    /// Returns a string that represents the Idf object, with key-value pairs separated by commas.
    ...
    ///
    /// let mut idf = Idf::new();
    /// idf.set_string(&1, String::from("John"));
    /// idf.set_integer(&2, 30);
    /// idf.set_string(&3, String::from("New York"));
    ///
    /// let result = idf.to_string();
    /// ```
